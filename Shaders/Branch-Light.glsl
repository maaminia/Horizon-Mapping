...
layout(binding = 1) uniform sampler2D transmissionTexture;		// Red channel = transmission, green channel = specular
...
void main()
{
	...
	vec2 xmitSpecColor = texture(transmissionTexture, vertexTexcoord).xy;
	...
	vec3 diff = diffuseColor.xyz * clamp(dot(m, ldir), 0.0, 1.0) * 0.3183;

	// Calculate specular factor sat(N * H)^alpha.

	float softening = smoothstep(0.0, 1.0, dot(normal, ldir));
	float spec = xmitSpecColor.y * (pow(clamp(dot(m, hdir), 0.0, 1.0), 80.0) * softening);
